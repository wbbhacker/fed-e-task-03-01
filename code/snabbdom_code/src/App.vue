...
export default {
  ...
  created(){
    let originalDataRef = this.originalData;
    this.data = [
      originalDataRef[0],
      originalDataRef[1],
      originalDataRef[2],
  ...
  methods:{
    ...
    add:function(){
      let n = this.originalData[Math.floor(Math.random() * 10)]
    ...
    view(){
      return h('div', [
        ...
        h('div.list', { style: { height: this.totalHeight + 'px' } }, this.data.map(this.movieView)),
      ]);
    },
    movieView(movie){
      return h('div.row', {
    ...
    render(){
      ...
      this.totalHeight = this.data[this.data.length - 1].offset + this.data[this.data.length - 1].elmHeight;
      this.vnode = this.patch(this.vnode,this.view(this.data));
    }
  }
}
</script>
